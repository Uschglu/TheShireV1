...
class JeunePlantRepository(context: Context) {
     ...
     *    utiliser creerSemisAvecImpactStocks().
     */
 ...
 * 
 * Permet à l'UI de réagir de façon claire :
 *  - Succes → afficher une confirmation
 *  - ErreurSachetIntrouvable → proposer d'ajouter le sachet
 *  - ErreurStockInsuffisant → indiquer combien il manque
 */
sealed class ResultatCreationSemis {
     ...
     * 
     * @param id                 L'ID du semis créé.
     * @param grainesDecrementees Nombre de graines décrémentées (0 en mode projection).
     * @param modeReel           Vrai si l'opération s'est faite en mode réel.
     */
    data class Succes(
        val id: Long,
